...
function appleCounter(appleObj) {
  ...
  let counter = 0;
  objArr = Object.keys(appleObj); //iterate through all keys in object and save keys into an array

    objArr.filter(function (apple){ //iterate through keys in new array
      if(apple.toLowerCase().includes('apple')){  //if 'apple' string is found in array element then increase counter
        counter++;
      }
    });
